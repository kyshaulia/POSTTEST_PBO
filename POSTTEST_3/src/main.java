...
public class main {
    ...
    static void tambahHijab(Scanner input, ArrayList<Hijab> daftarHijab){
        ...
        Hijab h;
        if(pilih == 1){
            h = new Pashmina(nama, warna, stok, harga);
        } else if(pilih == 2){
            h = new SegiEmpat(nama, warna, stok, harga);
        } else if(pilih == 3){
            h = new Instan(nama, warna, stok, harga);
        } else if(pilih == 4){
            h = new Bergo(nama, warna, stok, harga);
        } else if(pilih == 5) {
            h = new Khimar(nama, warna, stok, harga);
        } else {
            String kategori = k.getKategori()[pilih - 1];
            h = new Hijab(nama, warna, kategori, stok, harga);
        }
    ...
    static void lihatHijab(ArrayList<Hijab> daftarHijab){
        if(daftarHijab.size() == 0) {
            ...
            for(int i = 0; i < daftarHijab.size(); i++) {
                ...
                daftarHijab.get(i).tampilkanData(); // polymorphism sederhana
                System.out.println("-----------------------");
            }
        }
    }
    ...
    static void updateHijab(Scanner input, ArrayList<Hijab> daftarHijab){
        ...
        if(index >= 0 && index < daftarHijab.size()){
            ...
            Hijab hBaru;
            if(pilih == 1){
                hBaru = new Pashmina(h.getNama(), h.getWarna(), h.getStok(), h.getHarga());
            } else if(pilih == 2){
                hBaru = new SegiEmpat(h.getNama(), h.getWarna(), h.getStok(), h.getHarga());
            } else if(pilih == 3){
                hBaru = new Instan(h.getNama(), h.getWarna(), h.getStok(), h.getHarga());
            } else if(pilih == 4) {
                hBaru = new Bergo(h.getNama(), h.getWarna(), h.getStok(), h.getHarga());
            } else if(pilih == 5) {
                hBaru = new Khimar(h.getNama(), h.getWarna(), h.getStok(), h.getHarga());
            } else {
                String kategori = k.getKategori()[pilih - 1];
                hBaru = new Hijab(h.getNama(), h.getWarna(), kategori, h.getStok(), h.getHarga());
            }
